...
if [ -n "$usage_data" ] && echo "$usage_data" | jq -e . >/dev/null 2>&1; then
    ...
    rate_lines+="${white}current${reset} ${five_hour_pct_color}${five_hour_pct_fmt}%${reset} ${dim}⟳${reset} ${white}${five_hour_reset}${reset}"
    ...
    rate_lines+="\n${white}weekly${reset}  ${seven_day_pct_color}${seven_day_pct_fmt}%${reset} ${dim}⟳${reset} ${white}${seven_day_reset}${reset}"
    ...
    if [ "$extra_enabled" = "true" ]; then
        ...
        extra_col="${white}extra${reset}   ${extra_pct_color}\$${extra_used}${dim}/${reset}${white}\$${extra_limit}${reset} ${dim}⟳${reset} ${white}${extra_reset}${reset}"
        rate_lines+="\n${extra_col}"
    fi
fi
